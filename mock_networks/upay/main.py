"""UPay Mock Server — SQLite-backed, persists across restarts."""
import asyncio
import os
...
from shared.mock_data import UPAY_USERS, BANK_CODES, normalize_pk_phone
from shared.mock_db import WalletDB

DB_PATH = os.path.join(os.path.dirname(__file__), "upay.db")

app = FastAPI(title="UPay Mock Server", version="2.0.0")
db = WalletDB(db_path=DB_PATH, seed_data=UPAY_USERS)
...
@app.get("/health")
async def health():
    return {"status": "ok", "server": "UPay Mock", "port": 9004, "db": DB_PATH}


@app.post("/api/v1/user/verify")
async def lookup_account(req: AccountLookupRequest):
    """Verify UPay account exists before transfer."""
    ...
    if not user:
        return {
            ...
            "message": "Mobile number is not registered with UPay",
        }
    ...
    if user["status"] != "active":
        return {
            ...
            "message": "UPay account is not active",
        }
    ...
    return {
        ...
        "account_tier": user.get("account_tier", "standard"),
    }


@app.post("/api/v1/transfer")
async def send_money(req: TransferRequest):
    """Transfer funds to a UPay wallet."""
    ...
    if not receiver:
        return {
            ...
            "message": "Receiver mobile not registered with UPay",
        }
    ...
    if receiver["status"] != "active":
        return {
            ...
            "message": "Receiver UPay account is not active",
        }
    ...
    if req.amount > remaining_limit:
        return {
            ...
            "message": f"Receiver daily limit exceeded. Remaining: PKR {remaining_limit:,.2f}",
        }

    db.credit(receiver_mobile, req.amount)
    db.add_daily_sent(receiver_mobile, req.amount)

    txn_id = f"UP{uuid4().hex[:12].upper()}"
    ...
    return {
        ...
        "message": f"PKR {req.amount:,.2f} transferred to {receiver['name']} on UPay",
    }


# ── BANK ACCOUNT ENDPOINTS ───────────────────────────────────────────────────

@app.post("/api/v1/bank/link")
async def link_bank_account(req: BankLinkRequest):
    """Link a bank account to a UPay wallet."""
    await asyncio.sleep(0.5)

    user = db.get(normalize_pk_phone(req.mobile_number))
    if not user:
        return {"success": False, "error_code": "USER_NOT_FOUND",
                "message": "No UPay account found for this mobile number."}
...
async def withdraw_to_bank(req: BankWithdrawRequest):
    ...
    if not linked:
        return {"success": False, "error_code": "BANK_ACCOUNT_NOT_LINKED",
                "message": "This bank account is not linked to your UPay wallet."}

    if user["balance"] < req.amount:
        return {"success": False, "error_code": "INSUFFICIENT_BALANCE",
                "message": f"Insufficient UPay balance. Available: PKR {user['balance']:,.2f}"}

    db.debit(user_mobile, req.amount)
    txn_id = f"UP-WD-{uuid4().hex[:10].upper()}"
    ...
    return {
        ...
        "status": "processing",
        "message": f"PKR {req.amount:,.2f} transfer to {linked['bank_name']} initiated.",
    }


@app.get("/api/balances")
async def all_balances():
    """Debug: show all UPay account balances."""
...
async def reset():
    ...
    db.reset_all()
    return {"success": True, "message": "UPay accounts reset to original state."}
